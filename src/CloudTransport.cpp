...
CloudTransport::CloudTransport(const String& read_url,
                               const String& write_url,
                               const String& auth,
                               uint32_t timeoutMs)
: _read_url(read_url), _write_url(write_url), _auth(auth), _timeout(timeoutMs) {}

TransportResult CloudTransport::exchange(const std::vector<uint8_t>& request, const bool isWrite) {
  TransportResult res;
  HTTPClient http;
  if(!isWrite) 
  {
